...
import { green, blue, lightBlue, orange, red, amber, grey } from '@material-ui/core/colors';
        ...
        <Card className={classes.root}
            style={
                props.scopeName ?
                    ...
                    : undefined}>
            <CardHeader className={classesHeader.root}
                style={
                    props.scopeName ?
                        {
                            top: "-30px",
            ...
            <CardFooter hidden={(props.IsShowFooter === undefined) ? true : (props.IsShowFooter === true ? false : true)}>
                {props.footer}
            </CardFooter>
        </Card>
    )
...
export const MR_Card_Header = (props) => {
    ...
    const GetHeader = async () => {
        ...
        let result: any = await AxiosGetJson("Header/GetHeaderName", { nMenuID: props.nMenuID });
        if (result) {
                    ...
                    <React.Fragment>
                        {
                            props.isModeEdit == true ?
                                sHeader ?
                                    ...
                                    &nbsp;
                                    <Link to={sRounter} style={{ color: 'inherit', textDecoration: 'inherit', verticalAlign: 'middle' }}>{sHeader}</Link>
                                        <KeyboardArrowRight style={{ verticalAlign: 'middle' }} />&nbsp;<label style={{ verticalAlign: 'middle' }}>{'Add/Edit'}</label>
                                    </Fragment>
                                    :
                                    <Fragment>
                                        <Icon style={{ verticalAlign: 'middle' }}>{sIcon}</Icon>
                                ...
                                props.isModeApprove == true ?
                                    sHeader ?
                                        <Fragment>
                                            ...
                                            &nbsp;
                                            <Link to={sRounter} style={{ color: 'inherit', textDecoration: 'inherit', verticalAlign: 'middle' }}>{sHeader}</Link>
                                            <KeyboardArrowRight style={{ verticalAlign: 'middle' }} />&nbsp;<label style={{ verticalAlign: 'middle' }}>{'Approve'}</label>
                                        </Fragment>
                                        :
                                        <Fragment>
                                            <Icon style={{ verticalAlign: 'middle' }}>{sIcon}</Icon>
...
export const MuiButtonBack1 = (props) => {
    ...
    return (
        <StyledButton name={props.name || "btnCancel"} variant="contained" size="medium" {...props}>
...
export const MuiButtonSave = (props) => {
    ...
    return (
        <MuiButtonblue size="medium" {...props}>
            <SaveAlt className={classes.extendedIcon} />Save
            {props.children}
        </MuiButtonblue>
    )
}
...
export const MuiButtonCancel = (props) => {
    ...
    return (
        <StyledButton name={props.name || "btnCancel"} variant="contained" size="medium" {...props}>
            <ClearOutlinedIcon className={classes.extendedIcon} />Cancel
            {props.children}
        </StyledButton>
    )
}
...
export const MuiButtonAdd = (props) => {
    ...
    return (
        <MuiButtonGreen size="medium" {...props} >
            <SaveIcon className={classes.extendedIcon} />{props.label ? props.label : 'Add'}
            {props.children}
        </MuiButtonGreen>
    )
}
...
export const MuiButtonClear = (props) => {
    const classes = useStylesIconButton();
    return (
        <StyledButton name={props.name || "btnClear"} variant="contained" size="medium" {...props}>
            <ClearOutlinedIcon className={classes.extendedIcon} />{props.label ? props.label : 'Clear'}
            {props.children}
        </StyledButton>
    )
}
export const MuiButtonDelete = (props) => {
    const classes = useStylesIconButton();
    return (
        <StyledButton name={props.name || "btnDelete"} variant="contained" size="medium" color="secondary" {...props}>
            <DeleteIcon className={classes.extendedIcon} />Delete
            {props.children}
        </StyledButton>
    )
}
...
export const DialogConfirm = (funcYes, funcNo?, title = "", message = "") => {
    Sweetalert.Confirm(title === "" ? AlertTitle.Confirm : title, message === "" ? "Do you want to save data ?" : message, funcYes, funcNo);
};
export const DialogConfirmDelete = (funcYes, funcNo?, title = "", message = "") => {
    Sweetalert.Confirm(title === "" ? AlertTitle.Confirm : title, message === "" ? "Do you want to delete data ?" : message, funcYes, funcNo);
};
...
export const MuiButton = (props) => {
    ...
    return (
        <MuiButtonCustom size="medium" {...props}>
            <ArrowBackIosIcon className={classes.extendedIcon} />{i18n('common.' + props.event)}
            {props.children}
        </MuiButtonCustom>
    )
}